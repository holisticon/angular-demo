...
describe('OrdersCommonService', () => {
    ...
    describe('placeOrder', () => {
        it('submits the given addition to the shopping cart to the backend', () => {
            ...
            const postRequest = httpController.expectOne(`http://localhost/orders`);
            ...
            postRequest.flush(null, {
                ...
                statusText: 'Created',
                headers: {
                    Location: `http://localhost/orders/${getId(order)}`
                }
            });

            const getRequest = httpController.expectOne(`http://localhost/orders/${getId(order)}`);
